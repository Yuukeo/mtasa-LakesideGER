...
addEventHandler("onElementClicked",bank_door,function(_,state,player)
	...
	if(bank_wird_ausgeraubt~=0 or aktionlaeuft)then
		if isTimer(bank_timer_global)then
			...
			outputChatBox ( "Du kannst in "..var1.." Minuten wieder ausrauben.", player, 200, 0, 0 )
		end
	...
	if isEvil(player) then
		...
		infobox ( player, "Nur für böse Fraktionisten!", 5000, 150, 0, 0 )
		return
	end
...
createBlip (2364.8955078125, 2377.5654296875, 0, 52, 2, 255, 0, 0, 255, 0, 200, getRootElement() )

function giveBankKeyCard(_,player)
	if not player then return end
	outputChatBox("Du hast den Bankdirektor getötet und erhältst die Schlüsselkarte! Öffne die Tür zum Tresor! ( Klicke darauf )",player,0,200,0)
	gotKey=player
	
	local target=player
	
	if getElementData(target,"Wanteds")<=3 then
		setElementData(target,"Wanteds",getElementData(target,"Wanteds")+3)
	else
		setElementData(target,"Wanteds",6)
	end
...
function exit_bank ( thePlayer, matchingDimension )
	...
	if getElementData (thePlayer, "gotdamoney") and not bereitsdraussen then
		setElementData ( bank_robber, "Money",tonumber(getElementData(bank_robber, "Money" )) + 45000 )
...
function info_bank_timer ()
	if bank_i == 0 then
		if isElementWithinColShape ( bank_robber, bank_rob_col ) then
			...
			setElementData (bank_robber, "gotdamoney", true)
			setTimer ( function ( player )
				if getElementData (player, "gotdamoney") then
					if not bereitsdraussen then
						outputChatBox ( "Das Geld hat sich in Luft aufgelöst!", player, 200, 0, 0 )
					end
					setElementData (player, "gotdamoney", nil)
				end
...
function bank_col_leave (ele)
	...
	if ele == bank_robber then
		...
		outputChatBox ( "Der Bankraub ist fehlgeschlagen. (Räuber hat Marker verlassen)", root, 125, 0, 0 )
...
function robber_die ()
	outputChatBox ( "Der Bankraub ist fehlgeschlagen. (Räuber ist gestorben)", root, 125, 0, 0 )
	removeEventHandler ( "onPlayerWasted", bank_robber, robber_die ) 
	killTimer ( bank_timer )
	setTimer ( function() aktionlaeuft = false end, aktionpuffer, 1)
	bank_robber = 0
...
function bank_rob ( player, _ )
	if isEvil ( player ) and isElementWithinColShape ( player, bank_rob_col ) then
		if bank_wird_ausgeraubt ~= 1 then
			...
			outputChatBox ( "Du hast einen Bankraub gestartet. Bleibe 7 Minuten bei den Geldsaecken.", player, 0, 125, 0 )
			...
			bank_robber = player
			bereitsdraussen = false
			outputLog ( "Bankraub - "..getPlayerName(player).." - "..settings.general.factionlvlnames[getElementData ( player, "Faction")], "aktion" )
			
			bank_timer = setTimer ( info_bank_timer, 60000, 7 )
...
function nehmeGeldAb ( player, cmd, target )
	if isOnDuty ( player ) then
		if target then
			...
			if targetpl then
				if getElementData ( targetpl, "gotdamoney" ) then
					if bereitsdraussen then
						if getElementData ( targetpl, "Money" ) >= 45000 then
							setElementData ( targetpl, "Money", getElementData ( targetpl, "Money" ) - 45000 )
						else 
							setElementData ( targetpl, "Money", 0 )
						end
						setElementData (targetpl, "gotdamoney", false)
