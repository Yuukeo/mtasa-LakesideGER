...
addEvent("open:rescueUI",true)
addEventHandler("open:rescueUI",root,function()
	if(isLoggedin())then
		if(getElementData(lp,"ElementClicked")==false)then
			if(not isElement(Lakeside.Window[1]))then
				setWindowData("add","cursor_clicked",true)
				Lakeside.Window[1]=dgsCreateWindow(GLOBALscreenX/2-250/2,GLOBALscreenY/2-300/2,250,300,"Rescue Team - Duty",false,tocolor(255,255,255),nil,nil,settings.general.guimaincolor,nil,settings.general.guibgcolor,nil,true)
				...
				dgsCreateImage(85,10,80,80,":"..settings.general.scriptname.."/Files/Images/CompanyLogo/RescueTeam.png",false,Lakeside.Window[1])
				...
				addEventHandler("onDgsMouseClick",Lakeside.Button[3],
					function(btn,state)
						if(btn=="left" and state=="down")then
							...
							triggerServerEvent("go:rescueonoffduty",lp,"Offduty")
						end
					end,
				false)
				addEventHandler("onDgsMouseClick",Lakeside.Button[2],
					function(btn,state)
						if(btn=="left" and state=="down")then
							dgsCloseWindow(Lakeside.Window[1])
							setWindowData("remove","cursor_clicked",true)
							triggerServerEvent("go:rescueonoffduty",lp,"Onduty")
						end
					end,
				false)
